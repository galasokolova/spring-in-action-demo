...
public class OrderHandler {
    ...
    private final TacoRepository tacoRepository;  // TacoRepository for saving Taco to db

    @Autowired
    public OrderHandler(TacoOrderService tacoOrderService, Validator validator, TacoRepository tacoRepository) {
    ...
    private Mono<ServerResponse> saveTacosAndProcessOrder(TacoOrder tacoOrder, ServerRequest request, org.springframework.web.server.WebSession session) {
        ...
        return Mono.when(
                        tacoOrder.getTacos().stream()
                                .map(tacoRepository::save) // Saving Taco to db
                                .toList()
                )
